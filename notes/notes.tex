...
%\section{Poincare} 
%Think about this more \cite{Mazzoli2007}.


\appendix
\section{Derivation of the dipole operator\label{sec:dipole}}
The central element Eq.~\ref{eq:K-H} is the transition operator ${\cal D}$, which Ament et al.\ write as
\begin{equation}
 {\cal D}= \frac{1}{im\omega_{\bf k}}\sum_{i=1}^N   e^{i{\bf k}\cdot {\bf r}_i}{\boldsymbol \epsilon}\cdot{\bf p}_i.
\label{eq:dipoleoperator}
\end{equation}
where ${\bf r}_i$ and ${\bf p}_i$ are the position and momentum operators for electron $i$ and the prefactor is introduced for convenience. In the dipole approximation, one replaces the electron position operator with the atom location ${\bf R}_i$ leaving ${\cal D}$ composed of a momentum operator multiplied by a phase factor depending on the lattice site 
\begin{equation}
  {\cal D}={\boldsymbol \epsilon} \cdot {\bf D} ~~~{\rm with} ~~~~
 {\bf D}= \frac{1}{im\omega_{\bf k}}\sum_{i=1}^N e^{i{\bf k} \cdot {\bf R}_i} {\bf p}_i.
 \label{eq:transoper}
\end{equation}

A transition from $\ket{g}$ to $\bra{n}$ is then written
\begin{align}
     \bra{n} {\bf D} \ket{g} &=
\sum_{i=1}^N \frac{e^{i{\bf k} \cdot {\bf R}_i}}{im\omega_{\bf k}}\bra{n}{\bf p}_i\ket{g}.
\nonumber
\end{align}
We can use the commutator rules of quantum mechanics $[{\bf r}_i,{\bf p}_i]=i\hbar$ to transform this into a form with operator ${\bf r}_i$ \footnote{To see this one also needs to know the chain rule for operators $[A,BC]=B[A,C] + [A,B]C$}
\begin{align}
\bra{n} {\bf D} \ket{g} &=\sum_{i=1}^N\frac{e^{i{\bf k} \cdot {\bf R}_i}}{\hbar \omega_{\bf k}}
\bra{n} [\frac{{\bf p}_i^2}{2m},{\bf r}_i] \ket{g} \nonumber \\
    &\cong \sum_{i=1}^N\frac{e^{i{\bf k} \cdot {\bf R}_i}}{\hbar \omega_{\bf k}} (E_n-E_g)\bra{n} {\bf r}_i\ket{g} \nonumber \\
    &\cong \sum_{i=1}^N e^{i{\bf k} \cdot {\bf R}_i} \bra{n} {\bf r}_i\ket{g}
\end{align}    



%A typical experimental RIXS geometry is shown in Fig.~\ref{geom}. Vertically ($\sigma$) or horizontally ($\pi$) polarized x-rays are incident at $\theta_i$ on the sample and scattered through angle .
